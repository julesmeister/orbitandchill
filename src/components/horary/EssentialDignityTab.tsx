...
export default function EssentialDignityTab({ chartData, analysisData, question }: EssentialDignityTabProps) {
  const [selectedPlanet, setSelectedPlanet] = useState<string | null>(null);
  const [selectedDegree, setSelectedDegree] = useState<string>('');
  const [activeSection, setActiveSection] = useState<'overview' | 'analysis' | 'almuten' | 'context'>('overview');
  
  // Extract actual planetary positions from chart data
  const getPlanetaryPositions = () => {
    const positions: { [key: string]: { sign: string; degree: number; house: number } } = {};
  ...
  const planetaryDignities = Object.entries(planetaryPositions).map(([planet, pos]) => 
  ...
  const DignityCard = ({ dignity, rank }: { dignity: any; rank: number }) => {
    const strengthInfo = getDignityStrengthDescription(dignity.overallAssessment);
    const contextual = contextualDignityInterpretation(dignity, question.question);
      ...
      >
        <div className="flex items-center justify-between mb-2">
          ...
          <Badge 
            ...
            backgroundColor={getStrengthStyle(strengthInfo.color).backgroundColor}
            textColor={getStrengthStyle(strengthInfo.color).color}
          />
        </div>
        
        <div className="text-sm mb-2">
          <span className="font-bold">{dignity.sign}</span> {Math.floor(dignity.degree)}°{String(Math.floor((dignity.degree % 1) * 60)).padStart(2, '0')}'
        </div>
        
        <div className="text-xs text-gray-700 mb-2">
          Score: {dignity.strengthScore} | {dignity.contextualMeaning.split('-')[0]}
        </div>
        
        <div className="space-y-1">
          {dignity.dignities.ruler && <Badge label="Ruler" backgroundColor={DIGNITY_COLORS.ruler} />}
          {dignity.dignities.exaltation && <Badge label="Exaltation" backgroundColor={DIGNITY_COLORS.exaltation} />}
          {dignity.dignities.triplicity && <Badge label="Triplicity" backgroundColor={DIGNITY_COLORS.triplicity} textColor="black" />}
          {dignity.debilities.detriment && <Badge label="Detriment" backgroundColor={DIGNITY_COLORS.detriment} />}
          {dignity.debilities.fall && <Badge label="Fall" backgroundColor={DIGNITY_COLORS.fall} />}
          {dignity.debilities.peregrine && <Badge label="Peregrine" backgroundColor={DIGNITY_COLORS.peregrine} />}
        </div>
    ...
    return (
      <div className="bg-white border border-black p-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div>
            <h4 className="font-space-grotesk font-bold mb-3">Position & Basic Info</h4>
            <div className="space-y-2 text-sm">
              <div><strong>Sign:</strong> {dignity.sign}</div>
              <div><strong>Degree:</strong> {Math.floor(dignity.degree)}°{String(Math.floor((dignity.degree % 1) * 60)).padStart(2, '0')}'</div>
              <div><strong>Dispositor:</strong> {dispositor}</div>
              <div><strong>Strength Score:</strong> {dignity.strengthScore}/15</div>
              <div><strong>Chart Type:</strong> {isDayChart ? 'Day' : 'Night'}</div>
            </div>
            
            <h4 className="font-space-grotesk font-bold mb-3 mt-6">Dignities</h4>
            <div className="space-y-1 text-sm">
              {dignity.dignities.ruler && <DignityBadge type="Ruler" backgroundColor={DIGNITY_COLORS.ruler} score="+5" />}
              {dignity.dignities.exaltation && <DignityBadge type="Exaltation" backgroundColor={DIGNITY_COLORS.exaltation} score="+4" />}
              {dignity.dignities.exaltationDegree && <div className="text-xs ml-5" style={{ color: '#6bdbff' }}>Super-exalted at exact degree!</div>}
              {dignity.dignities.triplicity && <DignityBadge type="Triplicity" backgroundColor={DIGNITY_COLORS.triplicity} score="+3" />}
              {dignity.dignities.term && <DignityBadge type="Term" backgroundColor={DIGNITY_COLORS.term} score="+2" />}
              {dignity.dignities.face && <DignityBadge type="Face" backgroundColor={DIGNITY_COLORS.face} score="+1" />}
            </div>
            
            <h4 className="font-space-grotesk font-bold mb-3 mt-6">Debilities</h4>
            <div className="space-y-1 text-sm">
              {dignity.debilities.detriment && <div className="flex items-center"><span className="w-3 h-3 bg-red-500 rounded mr-2"></span>Detriment</div>}
              {dignity.debilities.fall && <div className="flex items-center"><span className="w-3 h-3 bg-orange-500 rounded mr-2"></span>Fall</div>}
              {dignity.debilities.fallDegree && <div className="text-xs text-orange-600 ml-5">Super-debilitated at exact degree!</div>}
              {dignity.debilities.peregrine && <div className="flex items-center"><span className="w-3 h-3 bg-gray-500 rounded mr-2"></span>Peregrine</div>}
            </div>
          </div>
          
          <div>
            <h4 className="font-space-grotesk font-bold mb-3">Interpretation</h4>
            <InfoBox
            ...
            {contextual.overrides.length > 0 && (
              <ColoredBox backgroundColor="#f2e356" textColor="black" className="p-4 mb-4">
                <div className="font-bold text-sm mb-2">Context Overrides:</div>
                {contextual.overrides.map((override, index) => (
                  <div key={index} className="text-sm">{override}</div>
                ))}
              </ColoredBox>
            )}
            ...
            <ColoredBox backgroundColor="#51bd94" className="p-4">
              ...
              <div className="text-xs mt-2">
                <Badge 
                  ...
                  textColor={contextual.confidence === 'high' ? 'black' : contextual.confidence === 'medium' ? 'black' : 'white'}
                />
              </div>
            </ColoredBox>
          </div>
        ...
        {contradictions.hasContradictions && (
          <div className="mt-6 pt-6 border-t border-gray-200">
            <h4 className="font-space-grotesk font-bold mb-3">Contradictions Analysis</h4>
            <ColoredBox backgroundColor="#f0e3ff" textColor="black" className="p-4">
              <div className="font-bold text-sm mb-2">Mixed Dignities Found:</div>
        ...
        <div className="mt-6 pt-6 border-t border-gray-200">
          <h4 className="font-space-grotesk font-bold mb-3">Descriptive Qualities</h4>
  ...
  const AlmutenSection = () => {
    ...
    const almutenResult = calculateAlmuten(almutenSign, almutenDegree, isDayChart);
    
    return (
      <div className="space-y-6">
        <div className="bg-white border border-black p-6">
          <h3 className="font-space-grotesk font-bold text-xl mb-4">Almuten Calculator</h3>
          <p className="text-sm text-gray-700 mb-4">
            The almuten is the planet with the most essential dignity in any given degree.
            Use this when you cannot use the house ruler as significator.
          </p>
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-6">
            <div>
              ...
              >
                {ESSENTIAL_DIGNITY_TABLE.map(sign => (
                  <option key={sign.sign} value={sign.sign}>{sign.sign}</option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <input
                ...
                step="0.1"
                value={almutenDegree}
                onChange={(e) => setAlmutenDegree(parseFloat(e.target.value) || 0)}
                className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-black"
              />
            </div>
          </div>
          
          <ColoredBox backgroundColor="#e3f2fd" textColor="black" borderColor="blue-200" className="p-4">
            <div className="font-bold text-lg mb-2">
              Almuten of {Math.floor(almutenDegree)}°{String(Math.floor((almutenDegree % 1) * 60)).padStart(2, '0')}' {almutenSign}
            </div>
            <div className="text-2xl font-bold mb-2" style={{ color: '#1976d2' }}>{almutenResult.almuten}</div>
          ...
          <div className="mt-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {almutenResult.competitors.map(comp => (
                <div key={comp.planet} className={`p-2 border rounded text-center ${
                  comp.planet === almutenResult.almuten ? 'bg-blue-100 border-blue-300' : 'bg-white border-gray-300'
                }`}>
    ...
    <div className="space-y-6">
      ...
      {activeSection === 'overview' && (
        <div className="space-y-6">
          ...
          <ColoredBox backgroundColor="#e3f2fd" textColor="black" borderColor="blue-200" className="p-4">
            <h4 className="font-space-grotesk font-bold mb-2">{isDayChart ? 'Day' : 'Night'} Chart</h4>
            <p className="text-sm text-gray-700">
              Sun in house {sunData.house} - determines triplicity rulers.
              {isDayChart ? ' Using day triplicity rulers.' : ' Using night triplicity rulers.'}
            </p>
          </ColoredBox>

          {/* Strength ranking */}
          <div className="bg-white border border-black p-6">
            <h4 className="font-space-grotesk font-bold mb-4">Planetary Strength Ranking</h4>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {sortedDignities.map((dignity, index) => (
                <DignityCard key={dignity.planet} dignity={dignity} rank={index + 1} />
              ))}
            </div>
          </div>

          {/* Key principles */}
          <ColoredBox backgroundColor="#fffde7" textColor="black" borderColor="yellow-200" className="p-4">
            <h4 className="font-space-grotesk font-bold mb-3">The Three Keys to Judgment</h4>
            <div className="grid md:grid-cols-3 gap-4 text-sm">
              {[
                { title: 'Essential Dignity', description: 'Shows power to act' },
      ...
      {activeSection === 'analysis' && (
        <div className="space-y-6">
          <div className="bg-white border border-black p-6">
            <h3 className="font-space-grotesk font-bold text-xl mb-4">Detailed Dignity Analysis</h3>
            ...
            <div className="grid md:grid-cols-4 gap-4 mb-6">
              ...
              />
              <StatCard 
                value={sortedDignities.filter(d => d.overallAssessment === 'very weak' || d.debilities.detriment || d.debilities.fall).length}
                label="Debilitated"
              ...
              />
              <StatCard 
                value={sortedDignities.filter(d => d.debilities.peregrine).length}
                label="Peregrine"
                backgroundColor="#f5f5f5"
                textColor="#616161"
              ...
              />
              <StatCard 
                value={sortedDignities.filter(d => analyzeContradictions(d).hasContradictions).length}
            ...
            <div className="space-y-3">
              {sortedDignities.map((dignity, index) => {
                ...
                return (
                  <div key={dignity.planet} className="border border-gray-200 p-4">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center">
                        <span className="font-bold mr-2">#{index + 1} {dignity.planet}</span>
                        <span className="text-sm text-gray-600">{dignity.sign} {Math.floor(dignity.degree)}°</span>
                      </div>
                      <Badge 
                        label={strengthInfo.label}
                        backgroundColor={getStrengthStyle(strengthInfo.color).backgroundColor}
      ...
      {activeSection === 'context' && (
        <div className="bg-white border border-black p-6">
          <h3 className="font-space-grotesk font-bold text-xl mb-4">Contextual Interpretation</h3>
          
          <div className="space-y-4">
            <ColoredBox backgroundColor="#fff3e0" textColor="black" borderColor="orange-200" className="p-4">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <p className="text-sm mt-3 text-gray-700">
                  ...
                  Treat essentially strong planets as benefic, debilitated ones as malefic.
                </p>
              </div>
              ...
              <div>
                ...
                <ul className="text-sm space-y-2 text-gray-700">
                  <li>• Lost umbrella: Saturn in Cancer = wet barrier (descriptive)</li>
                  <li>• Beach weather: Jupiter in Pisces = rain god (contextual malefic)</li>
                  <li>• Job application: Planet in detriment = desperate, unqualified</li>
                  <li>• Relationship: Venus/Moon strong = attractive, confident</li>
                </ul>
              </div>
            </div>
