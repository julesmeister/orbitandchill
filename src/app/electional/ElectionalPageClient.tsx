...
export default function ElectionalPageClient() {
  ...
  // Load events based on tab - persisted events for bookmarked/manual, month events for others
  useEffect(() => {
    if (user?.id) {
      if (selectedTab === 'bookmarked' || selectedTab === 'manual') {
        // Load all persisted events (includes bookmarked and manual events)
