...
export default function ChartQuickActions({
  ...
  const { selectedPerson, defaultPerson, people, selectedPersonId, setSelectedPerson } = usePeopleAPI();
  ...
  const handlePersonSaved = useCallback((person: Person) => {
    ...
    // Sync both stores when person is saved
    setSelectedPerson(person.id);
    setGlobalSelectedPerson(person.id);
    onPersonChange?.(person);
  }, [setSelectedPerson, setGlobalSelectedPerson, onPersonChange]);

  const handleCancelAddPerson = useCallback(() => {
    setShowAddPersonForm(false);
  }, []);
  ...
  const handlePersonEdited = useCallback((person: Person) => {
    setShowEditPersonForm(false);
    setEditingPersonData(null);
    // Sync both stores when person is edited
    setSelectedPerson(person.id);
    setGlobalSelectedPerson(person.id);
    onPersonChange?.(person);
  }, [setSelectedPerson, setGlobalSelectedPerson, onPersonChange]);
  ...
  const handlePersonSelectWrapper = useCallback((person: Person | null) => {
        ...
        <div className="border-b border-black bg-white">
          ...
          <div className="grid grid-cols-3 gap-0 border-b border-black">
            ...
            <button
              ...
              className="group relative p-4 transition-all duration-300 hover:bg-black overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {/* Animated background gradient */}
