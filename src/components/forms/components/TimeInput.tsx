...
interface TimeInputProps {
  timeInput: { hours: string; minutes: string; period: 'AM' | 'PM' };
  ...
  return (
    ...
    >
      <label className="synapsas-label mb-3 block">
        Time of Birth {required && <span className="text-red-500">*</span>}
      </label>
              ...
              setTimeout(() => {
                if (!minutesDropdownRef.current?.contains(document.activeElement)) {
                  ...
                  if (value.length === 1 && /^\d$/.test(value)) {
                    onTimeChange('minutes', value.padStart(2, '0'));
                  }
                }
              }, 150);
            }}
