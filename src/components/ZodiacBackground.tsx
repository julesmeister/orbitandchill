...
// Extract the zodiac symbol rendering function from InteractiveNatalChart
const renderZodiacSymbol = (symbol: string, size: number = 20) => {
...
const ZodiacBackground: React.FC<ZodiacBackgroundProps> = ({ 
  className = ""
}) => {
  ...
  // Match each zodiac symbol to its specific color
  const zodiacColorMap: { [key: string]: string } = {
    "♈": "#ff91e9", // Aries - purple
    "♉": "#4ade80", // Taurus - green
    "♊": "#f2e356", // Gemini - yellow
    "♋": "#6bdbff", // Cancer - blue
    "♌": "#ff8c42", // Leo - orange
    "♍": "#f0e3ff", // Virgo - light purple
    "♎": "#ff6b6b", // Libra - coral
    "♏": "#4ecdc4", // Scorpio - teal
    "♐": "#45b7d1", // Sagittarius - sky blue
    "♑": "#96ceb4", // Capricorn - mint
    "♒": "#feca57", // Aquarius - golden
    "♓": "#ff9ff3", // Pisces - pink
  };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      // Move to next symbol
      setCurrentSymbol((prev) => (prev + 1) % zodiacSymbols.length);
      
      // Generate position strongly biased towards far edges
      let x;
      
      // 90% chance for far edges, 10% chance for middle
      if (Math.random() < 0.9) {
        // Position on far edges where there's less text
        if (Math.random() < 0.5) {
          x = 2 + Math.random() * 12; // Far left edge (2-14%)
        } else {
          x = 86 + Math.random() * 12; // Far right edge (86-98%)
        }
      } else {
        // Occasionally in middle but very rare
        x = 35 + Math.random() * 30; // Limited middle area (35-65%)
      }
      
      // Keep within safe vertical bounds - avoid top and bottom edges
      const y = 35 + Math.random() * 30; // Use 35-65% height (safe middle zone)
      
      setSymbolData({
        x,
        y,
        size: 3 + Math.random() * 2, // Size between 3-5
      });
    }, 1500); // Change symbol every 1.5 seconds

    return () => clearInterval(interval);
  }, [zodiacSymbols.length]);

  const symbol = zodiacSymbols[currentSymbol];
  const color = zodiacColorMap[symbol];
  ...
  return (
    <>
      ...
      <div className={`absolute inset-0 overflow-hidden pointer-events-none ${className}`}>
        ...
        >
          <g 
            key={`${currentSymbol}-${symbolData.x}-${symbolData.y}`}
            transform={`translate(${symbolData.x}, ${symbolData.y})`}
            className="zodiac-single"
            style={{ color }}
          >
            {renderZodiacSymbol(symbol, symbolData.size)}
          </g>
        </svg>
      </div>
    </>
